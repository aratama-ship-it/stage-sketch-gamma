...
`index.js` とこのフォルダの同梱ローダーが、舞台スケッチで「見本のショー」として開けるデータの保存先です。
...
`romeo-juliet-cued.js` は、41灯・4トラス／33場面・47本のセリフキュー・19本の音楽キュー・33本のライトキューを持つ読み取り専用の完成見本です。初回起動時にショー一覧へ追加し、同じIDの利用者編集済みショーは上書きしません。棚の保存が容量不足で失敗したときは、既存ショーを変えず追加を止めます。台詞、時間、照明・舞台の指示は稽古・会場安全・上演GOを確定するものではなく、サンプル上の提案／仮値を含みます。

`romeo-juliet-second.json` は別IDの「ロミオとジュリエット｜RJセカンド（受け渡す手）」の編集用正本です。5章・34場面、10人の兼役、床上の身体表現、配置・照明の意図、独自翻案の台詞、169本のキューを持ちます。`node stage-samples/build-romeo-juliet-second.mjs` で `romeo-juliet-second.js` を生成し、起動時にショー一覧へ追加します。既存のロミオとジュリエット見本と保存済みショーは置き換えません。照明の仕込み、技、タイミング、上演の判断は稽古と実会場で確認する前の案です。
...
- RJセカンド: `romeo-juliet-second.json` の各場面 `rehearsal.holdDurationSeconds` / `transitionToNextSeconds`。章の `timelineDurationSeconds` は場面時間と転換の合計。
