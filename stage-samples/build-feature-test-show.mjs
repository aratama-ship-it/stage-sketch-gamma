...
section("c", "C 舞台セット・小道具・空中");
{
  ...
  scene("c1", "C-1 床に置く物すべて", 1,
    `${CHECK}登録できる床物を1つずつ。手前の2脚では演者01が座り、演者02が座面に立つ。どちらも選ぶと姿勢一覧に「座る」と「立つ」があり、椅子から下ろすと「座る」は一覧から消える。正面図の実寸（人と比べる）、平面図の足元、「枠の壁」の穴と枠幅0.25m、車の向き。壁は厚み固定。壁（3×2.5）は映す絵（格子模様・project.photos の再利用）を持たせてあり、正面図で縦横比を保ったまま中央へ映る（切らずにレターボックス）。隣の「枠の壁（フレーム）」には絵を付けていない（穴の向こうに絵が浮くため対象外＝操作パネルにも出ない）。この2つを同じ画面で「映る／映らない」を見比べる。正面図と平面図で大道具をクリックし、図に添える設定のオン・オフ、1列・2列・3列表示、図のスクロール、別タブからの復帰、物をドラッグして離した後でも選択パネルが見え、操作できることを確認する。パネルの列間を横にドラッグし、2列・3列・1列の各表示で幅が変わること、列をスクロールしても取っ手を使えること、再読み込み後も幅が残ること、左右キーで調整しEnterで元に戻せることを確認する。`, pieces);
}
{
  const pieces = [];
