...
section("c", "C 舞台セット・小道具・空中");
{
  ...
  scene("c1", "C-1 床に置く物すべて", 1,
    `${CHECK}登録できる床物を1つずつ。電球（点光源）は大道具で登録し、点灯・光色・明るさ・届く距離を調整する。正面図・平面図・3Dで光源と周囲の演者／道具の変化、消灯、保存と再読み込みを確認する。照明デザインの固定灯には加わらない。手前の2脚では演者01が座り、演者02が座面に立つ。どちらも選ぶと姿勢一覧に「座る」と「立つ」があり、椅子から下ろすと「座る」は一覧から消える。正面図の実寸（人と比べる）、平面図の足元、「枠の壁」の穴と枠幅0.25m、車の向き。壁は厚み固定。壁（3×2.5）は映す絵（格子模様・project.photos の再利用）を持たせてあり、正面図で縦横比を保ったまま中央へ映る（切らずにレターボックス）。隣の「枠の壁（フレーム）」には絵を付けていない（穴の向こうに絵が浮くため対象外＝操作パネルにも出ない）。この2つを同じ画面で「映る／映らない」を見比べる。正面図と平面図で大道具をクリックし、図に添える設定のオン・オフ、1列・2列・3列表示、図のスクロール、別タブからの復帰、物をドラッグして離した後でも選択パネルが見え、操作できることを確認する。パネルの列間を横にドラッグし、2列・3列・1列の各表示で幅が変わること、列をスクロールしても取っ手を使えること、再読み込み後も幅が残ること、左右キーで調整しEnterで元に戻せることを確認する。`, pieces);
}
{
  const pieces = [];
...
}
{
  ...
  rows[rows.length - 1].note += '\n劇場プレビューの袖幕: プロセニアム・扇形ホール・角形ホール・学校体育館・エンドステージ・ブラックボックス・歌舞伎舞台には、各規模で左右の袖が最初からあること。トラバースは左右の客席を避けて両端に袖があり、幕の向きも客席に合わせること。袖をすべて削除すると幕も消え、プリセットを適用し直すと初期の袖へ戻ること。袖の移動・サイズ変更・削除・取り消しと、丸い袖で平面図とプレビューの幕の位置が一致すること。張り出し式・360度では左右の袖幕を自動追加しないこと。';
  ...
  rows[rows.length - 1].note += '\n会場の外枠と表示: 劇場形式プリセットのグリッドでビッグトップ・シャピトーの外周が真円に見えること。ブラックボックスは小9m四方・中13m四方の外枠の内側に舞台・袖・客席があること。外枠の寸法変更、舞台と袖の移動、客席の移動、取り消し・やり直し、ライブラリ保存と書出し・再読込で外枠と配置が保たれること。左設定レーンが細くなり、平面図と立体が大きく表示され、入力や反映ボタンが使えること。';
  ...
  rows[rows.length - 1].note += '\n舞台袖とバックスクリーン: 舞台袖の内側だけでなく一番手前と奥にも幕があり、幕の表示切替がなく常に平面図・3Dに見えること。後方の退場用袖も布を壁扱いにせず、退場経路を残すこと。7番でバックスクリーンを横方向にドラッグすると白い面が平面図・正面・立体に現れ、天井高を変えると上端が追従すること。設置・取り外し・Undo/Redo・ライブラリ保存と再読込を確認し、未設置の旧会場には面が増えないこと。';
  rows[rows.length - 1].note += '\n3Dモードの視点登録: 自由カメラに移動・高さ・画角・プリセットの操作は残り、「この視点を登録」ボタンと登録ダイアログは出ないこと。以前の登録視点は正面図で引き続き選べること。新しい見る位置は劇場設定9番から作ること。';
  ...
  rows[rows.length - 1].note += '\n劇場設定5番の客席床高: 錆色の客席を選ぶ前から「舞台側の床高」「後方の床高」が見えること。未選択時は入力できず、客席区画を選ぶとその区画の値を表示・編集できること。別の区画を選ぶと値が切り替わり、選択を外すと空欄に戻ること。';
}
