...
プロンプト v0.2.11。舞台スケッチ γ（ガンマ）向けの公開用プロンプト配布・JSON点検ページです。MCPは配布しません。
...
公開ファイルは /public/ai-json/。公開URLは https://aratama-ship-it.github.io/stage-sketch-gamma/public/ai-json/ 。worker.jsのゲストallowlistは変更していません。本体への直接読み込みや改変したブラウザを強制的に防ぐものではありません。本体組み込み・サーバー側防御は別発注です。
