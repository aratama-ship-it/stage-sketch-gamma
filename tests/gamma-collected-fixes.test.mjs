...
test("collected UI fixes retain data while changing the visible controls", () => {
  ...
  assert.match(html, /stage-sketch\.js\?v=2026092433/);
  assert.match(serviceWorker, /CACHE_NAME = "stage-sketch-gamma-shell-v325"/);
...
test("lighting apply belongs to the LX cue panel and playback uses an accessible SVG toggle", () => {
  ...
  assert.match(workspace, /light-design\/index\.html\?embed=gamma&v=2026092433/);
  assert.match(worker, /stage-sketch-gamma-shell-v325/);
