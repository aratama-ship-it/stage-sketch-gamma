...
test("collected UI fixes retain data while changing the visible controls", () => {
  ...
  assert.match(html, /class="stage-app-version">0\.2\.10</);
  assert.match(html, /id="stage-release-v023-title">v0\.2\.3</);
  assert.match(html, /<meta name="stage-sketch-gamma-version" content="v0\.2\.10">/);
  ...
  assert.match(html, /stage-sketch\.js\?v=2026092517/);
  assert.match(serviceWorker, /CACHE_NAME = "stage-sketch-gamma-shell-v359"/);
...
test("lighting apply belongs to the LX cue panel and playback uses an accessible SVG toggle", () => {
  ...
  assert.match(worker, /stage-sketch-gamma-shell-v359/);
