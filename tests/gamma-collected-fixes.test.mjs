...
test("collected UI fixes retain data while changing the visible controls", () => {
  ...
  assert.match(html, /stage-sketch\.js\?v=2026092355/);
  assert.match(serviceWorker, /CACHE_NAME = "stage-sketch-gamma-shell-v278"/);
...
test("lighting apply belongs to the LX cue panel and playback uses an accessible SVG toggle", () => {
  ...
  assert.match(worker, /stage-sketch-gamma-shell-v278/);
