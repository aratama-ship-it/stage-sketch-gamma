...
test("the app shell advances with the storage transaction code", () => {
  ...
  assert.match(stageHtml, /stage-sketch\.js\?v=2026092633/);
  assert.match(serviceWorker, /stage-sketch-gamma-shell-v389/);
...
test("indoor standing reception venue keeps its 3D room layout outside show data", () => {
  ...
  assert.match(stageHtml, /stage-first-person\.js\?v=2026092633/);
  assert.match(serviceWorker, /stage-venues\.js\?v=2026092525/);
  assert.match(serviceWorker, /stage-first-person\.js\?v=2026092633/);
});
