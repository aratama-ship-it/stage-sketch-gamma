...
test("the app shell advances with the storage transaction code", () => {
  ...
  assert.match(stageHtml, /style\.css\?v=2026092416/);
  assert.match(stageHtml, /stage-sketch\.js\?v=2026092415/);
  assert.match(serviceWorker, /stage-sketch-gamma-shell-v316/);
  assert.match(serviceWorker, /\.\/stage-project-backup-store\.js\?v=2026092213/);
  assert.match(serviceWorker, /\.\/style\.css\?v=2026092416/);
  assert.match(serviceWorker, /\.\/stage-sketch\.js\?v=2026092415/);
});
...
test("indoor standing reception venue keeps its 3D room layout outside show data", () => {
  ...
  assert.match(stageHtml, /stage-first-person\.js\?v=2026092350/);
  assert.match(serviceWorker, /stage-venues\.js\?v=2026092224/);
  assert.match(serviceWorker, /stage-first-person\.js\?v=2026092350/);
});
