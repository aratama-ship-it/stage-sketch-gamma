...
function diagram(p,assignment=null,empty=false){
 ...
 return `<svg viewBox="0 0 1000 650" role="img" aria-label="${p.count}人・${p.name}の平面図"><rect x="${500-stageW/2}" y="${305-stageD/2}" width="${stageW}" height="${stageD}" fill="#0d0e10" stroke="#382f25" stroke-width="2"/><path d="M500 ${305-stageD/2}V${305+stageD/2}" stroke="#382f25" stroke-width="2" stroke-dasharray="8 8"/><path d="M${500-stageW/2} ${305+stageD/2}H${500+stageW/2}" stroke="#a89b86" stroke-width="4"/><text x="50" y="52" font-size="30">奥</text><text x="500" y="632" text-anchor="middle" font-size="30">客席</text>${(empty?[]:p.slots).map((s,i)=>{const v=point(s);return `<circle cx="${v.x}" cy="${v.y}" r="${r}" fill="#c9a961"/>${assignment?`<text x="${v.x}" y="${v.y}" style="fill:#1a1409;font-size:${Math.min(19,r*.95)}px;font-weight:700" text-anchor="middle" dominant-baseline="central">${memberNumber(assignment[i])}</text>`:''}`}).join('')}</svg>`;
}
