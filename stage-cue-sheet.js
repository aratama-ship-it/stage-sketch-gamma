(function (root) {
   ...
   * 表の行は「シーン｜キュー｜メモ」のキュー単位（照明・音楽・セリフ）。語は「場面」でなく「シーン」。 */
  const DEPARTMENTS = [
    { key: "light", label: "照明" },
  ...
  function buildPerformerSheet(project, castKey, helpers = {}) {
    ...
    const t = (key) => translate(helpers, key);
    const columns = [
      ["scene", "シーン", "7%"], ["title", "題", "12%"], ["present", "いる／いない", "6%"], ["position", "立ち位置", "10%"],
      ["route", "動線", "7%"], ["pose", "ポーズ・向き", "10%"], ["props", "持ち物", "9%"], ["handoffs", "受け渡し", "9%"],
      ["entranceExit", "出ハケ", "8%"], ["cues", "自分に関わるキュー", "12%"], ["notes", "コツ・注意", "10%"],
  ...
  function buildDepartmentSheet(project, dept, helpers = {}) {
    ...
    if (dept === "stage") {
      ...
      return baseDepartmentSheet(project, dept, helpers, [["scene", "シーン", "22%"], ["machinery", "機構", "18%"], ["sets", "舞台セット", "18%"], ["note", "転換メモ", "30%"], ["timing", "時間", "12%"]], rows);
    }
    if (dept === "props") {
